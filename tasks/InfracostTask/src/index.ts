...
async function run() {
    ...
    try {
        ...
        if(command === "output") {
            // const html = await JSDOM.fromFile(file);
            //const { document } = html.window;
            await saveOutput(tasks.getVariable('System.TeamFoundationCollectionUri'), tasks.getVariable('System.AccessToken'), tasks.getVariable('Build.SourceVersion'), {
                id: tasks.getVariable('Build.SourceVersion'),
                report: he.encode('<div>didier & devops</div>')
            })
        }
        

        if(returnCode > 0){
            tasks.setResult(tasks.TaskResult.Failed, tasks.loc("InfracostToolNonZeroExit"));
        }
...
const saveOutput = async (organization: string, token: string, id: string, document: any): Promise<void> => {

    const api = VSS.WebApi.createWithBearerToken(organization,token);   
    const data = await api.getExtensionManagementApi();
    var doc = await data.getDocumentByName('zeroandone','infracost-devops-extension','Default','Current','PR',id);
    if(!doc){
       doc = await data.createDocumentByName(document,'zeroandone','infracost-devops-extension','Default','Current','PR');
        ...
        doc = await data.updateDocumentByName(update,'zeroandone','infracost-devops-extension','Default','Current','PR');
    }
